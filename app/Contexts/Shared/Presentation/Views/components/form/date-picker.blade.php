...
<div x-data="{
    ...
    init() {
        this.$nextTick(() => {
            ...
            // Sincronizar cambios desde el backend hacia Flatpickr sin reinicializar
        ...
        // Re-conectar Flatpickr si Livewire re-renderiza la sección del DOM
        Livewire.hook('commit', ({ component, commit, respond, succeed, fail }) => {
            succeed(() => {
                this.$nextTick(() => {
                    if (this.$refs.dateInput && !this.$refs.dateInput._flatpickr) {
                        this.initFlatpickr();
                    }
                });
            });
    ...
    <div class="relative custom-datepicker" wire:ignore>
        <input
            x-ref="dateInput"
            type="text"
            id="{{ $id }}"
        ...
        <div class="absolute top-1/2 right-4 z-10 -translate-y-1/2 pointer-events-none flex items-center gap-2">
            ...
            <svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24" fill="none" class="size-5 text-gray-500 dark:text-gray-400">
